...
async def my_action_method(ctx: kernel.KernelContext, payload):
...
Action methods can return values, which will be included in the [invocation's](/apps/invoke#return-values) final response. The examples above show actions returning data.
...
async def get_page_title(ctx: kernel.KernelContext, payload):
