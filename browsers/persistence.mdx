...
With Kernel, you can specify a browser to be persisted and then reuse that exact browser instance again. Using [standby mode](/launch/browser-standby), we put the browser to sleep in between connections, so you don't pay for idle clock time.
...
Persisted browsers exist indefinitely until you [delete them](/launch/browser-termination). See [here](/info/pricing) for plan limits on persistence.
