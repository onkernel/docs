...
async def main():
    kbrowser = await kernel.browsers.create()
...
For more complex scenarios, you can also use the [list files](/api-reference/browsers/list-files-in-a-directory) API together with [read file](/api-reference/browsers/read-file-contents) to enumerate and save all downloads at the end of a session.
