...
Browser pools let you maintain a set of identical, pre-configured browsers ready for immediate use. Instead of waiting for browser creation on each request, you can acquire a browser from the pool without the startup delay and release it when done.
...
For one-off or long-running browser sessions, [creating browsers directly](/browsers/create-a-browser) with an optional extended timeout may be simpler.
