...
For sites requiring 2FA, include the `code` field with a TOTP generator:

```typescript
values: {
  email: 'user@example.com',
  password: 'secretpassword',
  code: '123456',  // Generate with a TOTP library
}
```

<Tip>
When using [Auto-Login](/agent-auth/auto-login) with 2FA, the system will automatically use the `code` value when a 2FA field is detected. Use a TOTP generator library to provide the current code.
</Tip>
