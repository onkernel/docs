...
<Steps>
  ...
  <Step title="Start an Invocation">
    ...
    **Invocation states:**
    ...
    - `step`: `initialized` → `discovering` → `awaiting_input` ⇄ `submitting` | `awaiting_external_action` → `completed`
  </Step>
...
- **SSO/OAuth support** - Automatic handling of "Sign in with Google/GitHub/Microsoft" buttons
