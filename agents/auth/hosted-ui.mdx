...
| `allowed_domains` | No | Additional domains allowed during auth flow (e.g., `['accounts.google.com']` for SSO) |
...
2. Find and display the login form (including SSO buttons like "Sign in with Google")
3. Let the user enter credentials or click an SSO button
4. Handle multi-step auth (2FA, OTP, etc.)
5. Save the authenticated session to the profile

<Note>
If the login page has SSO buttons, users can click them directly. For SSO flows that redirect to external domains (e.g., Google, GitHub), include those domains in `allowed_domains` when creating the auth agent.
</Note>
