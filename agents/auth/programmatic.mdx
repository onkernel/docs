...
<Tip>
**Have credentials stored?** If you just want automated login with pre-linked credentials, [Auto-Login](/agent-auth/auto-login) is simpler—it handles everything automatically. Use Programmatic when you need custom control over each step.
</Tip>
...
<Steps>
  ...
  <Step title="Poll Until Fields Are Discovered">
    ...
    while (state.step === 'initialized' || state.step === 'discovering') {
      await sleep(2000);
  ...
  <Step title="Poll for Result">
    ```typescript
    // Poll until step changes from 'submitting'
    while (state.step === 'submitting' || state.step === 'awaiting_input') {
      state = await kernel.agents.auth.invocations.retrieve(invocation.invocation_id);
...
if (state.status === 'SUCCESS') {
  ...
  // Step 2: Poll until fields are discovered
  let state = await kernel.agents.auth.invocations.retrieve(invocation.invocation_id);
    ...
    <form onSubmit={(e) => {
      ...
      {pendingFields.map((field) => (
        <div key={field.name}>
          ...
          <input
            type={field.type === 'totp' ? 'text' : field.type}
            name={field.name}
            placeholder={field.placeholder}
            required={field.required}
            autoComplete={field.type === 'totp' ? 'one-time-code' : undefined}
