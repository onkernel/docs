...
Our Credentials Vault allows you to securely store login information and automate workflows end-to-end. When combined with [Agent Auth](/agents/auth/overview), your Kernel browsers stay automatically logged in 24/7 without additional user input.

## Why use Kernel's Credentials Vault?

Without stored credentials, every time a browser session expires, you need to re-collect credentials. With our credentials vault:
...
When creating Agent Auth invocations, specify `save_credential_as` with a unique identifier. This stores the credentials submitted in the Auth Agent to our vault.
...
#### `save_credential_as`
If you've stored credentials via `save_credential_as`, all subsequent browsers loaded with the associated [Auth Agent Profile](/agents/auth/hosted-ui#1-create-an-auth-agent) will be automatically logged in. We regularly refresh the profile with the stored credentials to ensure it is valid and authenticated.
...
const agent = await kernel.agents.auth.create({
  ...
  credential_name: credential.name,  // Credentials the Auth Agent should use to attempt login
});
```
...
| **Never shared with LLMs** | Values are never passed to LLMs |
...
- For sites that use authenticator apps (TOTP) for 2FA, you can provide a `totp_secret` to enable fully automated login.
...
3. **Monitor auth status** - Check auth agent status to detect when re-auth is needed
