...
- Added an API to get the current cursor position, useful for debugging and building adaptive automation logic.
- Added an [API](/api-reference/invocations/list-browsers-for-an-invocation) to retrieve browser session IDs associated with a specific invocation.

### Browsers and dashboard
- Added [SSH access](/browsers/ssh) to Kernel browsers, so you can connect directly via terminal for debugging and inspection.
- Added support for the 1280x800 viewport size, which improves click coordinate accuracy.

### Bug fixes
- Live View clipboard now syncs correctly between browser and local machine.
- Chrome address autofill prompts are automatically disabled.
