...
<Update label="January 16" tags={["Product"]}>
## Product updates
- Added a Past Sessions view to the [dashboard](https://dashboard.onkernel.com/) that displays your last 100 browser sessions, with a tooltip providing context about the session history limit.
- Fixed an issue where downloading multiple files with the same filename would overwrite previous downloads. Downloads now behave like local browsers, where subsequent files with identical names receive unique suffixes.
- Resolved browser session stability issues including premature disconnections and websocket failures. Added auto-retry logic for CDP connections and heartbeat mechanisms to keep sessions alive.
</Update>
