...
1. Include your API key in the `Authorization` header as a Bearer token:
   ```
   Authorization: Bearer YOUR_KERNEL_API_KEY
   ```

2. The server automatically detects non-JWT tokens and treats them as API keys.

3. API key authentication is useful for:
